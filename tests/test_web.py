...
class TestDebugAccessLogger:
    ...
    def test_log_success(self):
        ...
        request = make_mocked_request("GET", "/test", headers={"User-Agent": "TestAgent"})
    ...
    def test_log_no_user_agent(self):
...
class TestWebServer:
    ...
    def test_init_with_env_vars(self):
        ...
        with patch.dict(os.environ, {"ALFEN_WEB_HOST": "10.0.0.1", "ALFEN_WEB_PORT": "8090"}):
            server = WebServer(driver)
    ...
    @pytest.mark.asyncio
    async def test_run_on_glib_success(self):
        ...
        with patch("gi.repository.GLib.idle_add", mock_idle_add):
    ...
    @pytest.mark.asyncio
    async def test_run_on_glib_no_loop(self):
    ...
    @pytest.mark.asyncio
    async def test_handle_status_with_lock(self):
    ...
    @pytest.mark.asyncio
    async def test_handle_status_without_lock(self):
    ...
    @pytest.mark.asyncio
    async def test_handle_status_exception(self):
    ...
    @pytest.mark.asyncio
    async def test_handle_get_schema(self):
        """Test handle_get_schema returns config schema."""
        driver = Mock()
        server = WebServer(driver)
        request = make_mocked_request("GET", "/api/config/schema")
        
        with patch("alfen_driver.web.get_config_schema", return_value={"test": "schema"}):
    ...
    @pytest.mark.asyncio
    async def test_handle_put_config_success(self):
        ...
        data = json.loads(response.text)
        assert data == {"ok": True}
        mock_run.assert_called_once_with(driver.apply_config_from_dict, {"new": "config"})

    @pytest.mark.asyncio
    async def test_handle_put_config_validation_error(self):
        """Test handle_put_config with validation error."""
        driver = Mock()
        driver.apply_config_from_dict = Mock(return_value={"ok": False, "error": "Invalid value"})
    ...
    @pytest.mark.asyncio
    async def test_handle_put_config_invalid_json(self):
    ...
    @pytest.mark.asyncio
    async def test_handle_startstop_enable(self):
    ...
    @pytest.mark.asyncio
    async def test_handle_startstop_disable(self):
    ...
    @pytest.mark.asyncio
    async def test_handle_set_current(self):
        ...
        data = json.loads(response.text)
        assert data == {"ok": True}
        mock_run.assert_called_once_with(driver.set_current_callback, "/SetCurrent", 16.5)
    ...
    @pytest.mark.asyncio
    async def test_create_app_with_static_dir(self):
    ...
    @pytest.mark.asyncio
    async def test_create_app_without_static_dir(self):
