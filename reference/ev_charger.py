...
class EV_Charger(device.ModbusDevice):
    ...
    def device_init(self):
        ...
        self.data_regs = [
            ...
            Reg_text(
                5027, 22, "/CustomName", little=True, encoding="utf-8", write=True
            ),
        ...
        if self.have_display:
            self.data_regs.append(Reg_u16(5050, "/EnableDisplay", write=(0, 1)))

        if fwver < (0, 0x01, 0x22, 0x02):
            return

        self.data_regs += [Reg_u16(5062, "/MinCurrent", 1, "%d A", write=True)]
...
probe.add_handler(
    probe.ModelRegister(Reg_u16(5000), models, methods=["tcp"], units=[1])
)
