...
def main():
    ...
    print("Connected to Alfen charger. Polling data every 5 seconds. Press Ctrl+C to stop.")
    ...
    try:
        while True:
            try:
                ...
                decoder = BinaryPayloadDecoder.fromRegisters(rr.registers, byteorder=">", wordorder=">")
                v1 = decoder.decode_32bit_float()
                v2 = decoder.decode_32bit_float()
                v3 = decoder.decode_32bit_float()
                print(f"Voltages: L1={v1 if not math.isnan(v1) else 0:.2f}V, L2={v2 if not math.isnan(v2) else 0:.2f}V, L3={v3 if not math.isnan(v3) else 0:.2f}V")
                ...
                print(f"Currents: L1={i1 if not math.isnan(i1) else 0:.2f}A, L2={i2 if not math.isnan(i2) else 0:.2f}A, L3={i3 if not math.isnan(i3) else 0:.2f}A")
                ...
                print(f"Configured Current: {curr_config if not math.isnan(curr_config) else 0:.2f}A")
